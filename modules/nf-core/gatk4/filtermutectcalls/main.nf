process GATK4_FILTERMUTECTCALLS {
    ...
    tuple val(meta), path("*filtered.vcf.gz"), path("*filtered.vcf.gz.tbi"), emit: forMutect2Combine
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    echo "" | gzip > ${prefix}_filtered.vcf.gz
    touch ${prefix}_filtered.vcf.gz.tbi
    touch ${prefix}_filtered.vcf.gz.filteringStats.tsv
